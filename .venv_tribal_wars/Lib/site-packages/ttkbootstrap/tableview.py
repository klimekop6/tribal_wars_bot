...
class TableRow:
    ...
    def __init__(self, tableview, values, row_in_source):
        """
        Parameters:
        ...
        self.view: ttk.Treeview = tableview.view
        self._values = values
        self._row_in_source = row_in_source
        self._iid = None
    ...
    def build(self):
        ...
        if self._iid is None:
            ...
            self._row_in_source["id"] = self._iid
...
class Tableview(ttk.Frame):
    ...
    def __init__(
        ...
        Parameters:
            ...
            datasource (List):
                Data source used in coldata and rowdata.
    ...
    def build_table_data(self, coldata, rowdata):
        ...
        for values, row_in_source in zip(rowdata, self._datasource):
            if "id" in row_in_source:
                row_in_source["id"] = ""
            self.insert_row(values=values, row_in_source=row_in_source)
    ...
    def insert_row(self, index=END, values=[], row_in_source=[]) -> TableRow:
        ...
        record = TableRow(self, values, row_in_source)
    ...
    def delete_datasource(self):
        "Delete datasource and all corresponding rows"
    ...
    def _build_search_frame(self):
        ...
        ttk.Label(frame, text=MessageCatalog.translate("Search")).pack(
            side=LEFT, padx=5
        )
    ...
    def _build_pagination_frame(self):
        ...
        self.bin = ttk.PhotoImage(file="icons//bin.png")

        ttk.Button(
            pageframe,
            image=self.bin,
            command=self.delete_datasource,
    ...
    def _set_widget_binding(self):
        ...
        self.view.bind(
            "<Delete>", lambda _: self._rightclickmenu_cell.delete_selected_rows()
        )
...
class TableCellRightClickMenu(tk.Menu):
    ...
    def delete_selected_rows(self):
        ...
        if len(iids) > 0:
            ...
            to_del = []
            # Delete all selected list cells in _datasource
            for index, row in enumerate(self.master._datasource):
                if "id" in row and row["id"] in iids:
                    to_del.append(index)
                    if len(to_del) == len(iids):
                        break
            for index in reversed(to_del):
                del self.master._datasource[index]
