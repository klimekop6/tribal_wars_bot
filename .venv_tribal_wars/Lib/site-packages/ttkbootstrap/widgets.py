...
class DateEntry(ttk.Frame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # starting value
        self.entry.insert(tk.END, self._startdate.strftime(self._dateformat)[:-3])
    ...
    def _on_date_ask(self):
        ...
        try:
            ...
            self._startdate = datetime.today().strftime(self._dateformat)[:-3]
            self.entry.delete(first=0, last=tk.END)
            self.entry.insert(tk.END, self._startdate)
            _val = self._startdate
        ...
        self.entry.delete(first=0, last=tk.END)
        self.entry.insert(tk.END, new_date.strftime(self._dateformat)[:-3])
    ...
    *indeterminate* mode provides an animated display to let the
    ...
    def _set_mask(self):
        if self._traceid is None:
            self._traceid = self.variable.trace_add("write", self._set_widget_text)
...
class Meter(ttk.Frame):
    ...
    def _lookup_style_option(self, style: str, option: str):
        ...
        value = self.tk.call("ttk::style", "lookup", style, "-%s" % option, None, None)
        return value
