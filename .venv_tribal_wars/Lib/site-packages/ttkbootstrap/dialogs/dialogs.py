...
class DatePickerDialog:
    ...
    def _set_bindings(self):
        def on_mouse_leave(event=None):
            if event.widget != self.root:
                return
            self.root.bind(
                "<Button>", lambda _: self.root.after(50, self.root.destroy())
            )

        def on_mouse_enter(event=None):
            self.root.unbind("<Button>")

        self.root.bind("<Leave>", on_mouse_leave)
        self.root.bind("<Enter>", on_mouse_enter)
        self.root.bind("<Button>", lambda _: self.root.destroy())
    ...
    def _draw_electric_clock(self):
        def on_button_release(event, master, funcid) -> None:
            ...
            widget.focus()
            widget.unbind("<ButtonRelease-1>", funcid=funcid)
            ...
            def on_leave(event=None) -> None:
                ...
                funcid = widget.bind(
                    "<ButtonRelease-1>",
                    lambda event: on_button_release(event, master, funcid),
                    add="+",
                )
        ...
        def _update_time_entry(event=None):
            self.time_entry.delete(0, END)
            time = (
                f"{self.hours.get():>02}"
                + ":"
                + f"{self.minutes.get():>02}"
                + ":"
                + f"{self.seconds.get():>02}"
                + ":"
                + f"{self.milisec.get():>03}"
            )
            self.time_entry.insert(0, time)

        def _update_from_time_entry(event=None):
            time = self.time_entry.get().split(":")
            self.hours.delete("0", "end")
            self.hours.insert("0", time[0])
            self.minutes.delete("0", "end")
            self.minutes.insert("0", time[1])
            self.seconds.delete("0", "end")
            self.seconds.insert("0", time[2])
            self.milisec.delete("0", "end")
            self.milisec.insert("0", time[3])
        ...
        def submit(event=None):
            ...
            second = time[6:8]
            microsecond = time[-3:]
            self.date_selected = datetime(
                year=self.date_selected.year,
                month=self.date_selected.month,
                day=self.date_selected.day,
                hour=int(hour),
                minute=int(minute),
                second=int(second),
        ...
        self.electric_clock = ttk.Frame(self.frm_calendar)
        self.electric_clock.pack(fill=BOTH, expand=YES)
        self.electric_clock.columnconfigure(0, weight=1)
        self.electric_clock.columnconfigure(1, weight=1)
        day_time = datetime.strftime(self.startdate, "%H:%M:%S:%f")[:-3]

        choosed_time = ttk.StringVar(self.electric_clock)
        ttk.Label(self.electric_clock, text="Czas").grid(
            row=0, column=0, pady=5, padx=(10, 5), sticky=W
        )
        self.time_entry = ttk.Entry(self.electric_clock, width=12, justify=CENTER)
        self.time_entry.grid(row=0, column=1, pady=5, padx=(5, 10), sticky=E)
        choosed_time.set(day_time)
        self.time_entry.insert(0, day_time)
        self.time_entry.bind(
            "<KeyPress>", lambda _: self.time_entry.after(25, _update_from_time_entry)
        )

        ttk.Label(self.electric_clock, text="Godzina").grid(
            row=1, column=0, pady=5, padx=(10, 5), sticky=W
        )
        self.hours = ttk.Spinbox(
            self.electric_clock,
            values=tuple(f"{_:>02}" for _ in range(24)),
            width=2,
            wrap=True,
            command=_update_time_entry,
        )
        self.hours.grid(row=1, column=1, padx=(5, 10), pady=5, sticky=E)
        self.hours.insert(0, day_time[:2])
        self.hours.bind("<Key>", lambda _: self.hours.after(25, _update_time_entry))
        self.hours_funcid = None
        self.hours.bind(
            "<Map>",
            lambda _: self.hours.bind(
                "<ButtonRelease-1>",
                lambda event: on_button_release(
                    event, self.electric_clock, self.hours_funcid
                ),
            ),
        )

        ttk.Label(self.electric_clock, text="Minuta").grid(
            row=2, column=0, pady=5, padx=(10, 5), sticky=W
        )
        self.minutes = ttk.Spinbox(
            self.electric_clock,
            values=tuple(f"{_:>02}" for _ in range(60)),
            width=2,
            wrap=True,
            command=_update_time_entry,
        )
        self.minutes.grid(row=2, column=1, padx=(5, 10), pady=5, sticky=E)
        self.minutes.insert(0, day_time[3:5])
        self.minutes.bind("<Key>", lambda _: self.hours.after(25, _update_time_entry))
        self.minutes_funcid = None
        self.minutes.bind(
            "<Map>",
            lambda _: self.minutes.bind(
                "<ButtonRelease-1>",
                lambda event: on_button_release(
                    event, self.electric_clock, self.minutes_funcid
                ),
            ),
        )

        ttk.Label(self.electric_clock, text="Sekunda").grid(
            row=3, column=0, pady=5, padx=(10, 5), sticky=W
        )
        self.seconds = ttk.Spinbox(
            self.electric_clock,
            values=tuple(f"{_:>02}" for _ in range(60)),
            width=2,
            wrap=True,
        ...
        self.seconds.grid(row=3, column=1, padx=(5, 10), pady=5, sticky=E)
        self.seconds.insert(0, day_time[6:8])
        self.seconds.bind("<Key>", lambda _: self.hours.after(25, _update_time_entry))
        self.seconds_funcid = None
        self.seconds.bind(
            "<Map>",
            lambda _: self.seconds.bind(
                "<ButtonRelease-1>",
                lambda event: on_button_release(
                    event, self.electric_clock, self.seconds_funcid
                ),
            ),
        )

        ttk.Label(self.electric_clock, text="Milisekunda").grid(
            row=4, column=0, pady=5, padx=(10, 5), sticky=W
        )
        self.milisec = ttk.Spinbox(
            self.electric_clock,
            values=tuple(f"{_:>03}" for _ in range(1000)),
            width=3,
            wrap=True,
            command=_update_time_entry,
        )
        self.milisec.grid(row=4, column=1, padx=(5, 10), pady=5, sticky=E)
        self.milisec.insert(0, day_time[-3:])
        self.milisec.bind("<Key>", lambda _: self.hours.after(25, _update_time_entry))
        self.milisec_funcid = None
        self.milisec.bind(
            "<Map>",
            lambda _: self.milisec.bind(
                "<ButtonRelease-1>",
                lambda event: on_button_release(
                    event, self.electric_clock, self.milisec_funcid
                ),
            ),
        )

        ttk.Button(
            self.electric_clock, text="Wyczyść", command=self.on_reset_date
        ).grid(row=5, column=0, padx=(10, 5), pady=(5, 10), sticky=W)

        submit_button = ttk.Button(self.electric_clock, text="Gotowe", command=submit)
        submit_button.grid(row=5, column=1, padx=(5, 10), pady=(5, 10), sticky=E)

        self.root.bind(
            "<Return>",
            lambda args: [
                self.root.focus_displayof().event_generate("<Leave>"),
                self.root.unbind("<Button>"),
                submit_button.event_generate("<Button-1>"),
                submit(),
            ],
        )
    ...
    def _draw_titlebar(self):
        ...
        self.title.bind("<Button-1>", lambda event: self._get_pos(event, "title"))
    ...
    def _selection_callback(func):
        ...
        def inner(self, *args):
            ...
            self.electric_clock.destroy()
            self._draw_calendar()
            self._draw_electric_clock()

        return inner

    def _get_pos(self, event, *args):
...
class Querybox:
    ...
    @staticmethod
    def get_color(parent=None, title="Color Chooser", initialcolor=None, **kwargs):
    ...
    @staticmethod
    def get_string(prompt="", title=" ", initialvalue=None, parent=None, **kwargs):
